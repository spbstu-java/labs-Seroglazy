...
public class Main {
    public static void main(String[] args) {
        ...
        try
        {
            ...
            float[][] rotArray = baseArray;

            for(int i = 0; i < 3; i++)
            {
                rotArray = rotateArray(rotArray, _size);
                if (i>0){
                    writeArrayToFile(divArray(rotArray, _size), _size, true);
                }
                else{
                    writeArrayToFile(divArray(rotArray, _size), _size, false);
                }
            }
        }
    ...
    static void writeArrayToFile (float[][] array, int size, boolean rewrite) {
        try(FileWriter writer = new FileWriter("output.txt", rewrite))
        {
            for (int i = 0; i < size; i++) {
                for (int j = 0; j < size; j++) {
