...
public class Main {
    ...
    {
        loadDictionary();
        System.out.println("Переводимый текст:");
        loadTranslatableData();

        System.out.println("Перевод по предложениям:");
    ...
    public static void loadTranslatableData()
    {
        try(BufferedReader br = new BufferedReader(new FileReader("text.txt"))) {
            ...
            while((str = br.readLine()) != null) {
                ...
                String[] inputSent = str.split( "\\.");
                for (String s: inputSent) {
                    translatable.add(s);
                }
            }
            System.out.println("");
    ...
    public static void findTranslate(String sentence) {
        ...
        for (int i = 0; i < inputArray.length; i++) {
            ...
            for (Map.Entry<String, String> e : dictionary.entrySet()) {
                if (e.getKey().startsWith(inputArray[i])
                        && e.getKey().length()>translate.length()
